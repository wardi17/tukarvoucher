...
class BerikanVocherController extends Controller
{
   ...
   public function listdata() {
    ...
    public function UpdateSatusPrint(){
               try {       
        // Retrieve data from the model
                $data = $this->tsberikanVocherModel->UpdateSatusPrint(); // Assuming this method exists in your model
                // Check if data is empty
                if (empty($data)) {
                    $this->sendJsonResponse([], 200); // Return an empty array if no data found
                    return;
                ...
                error_log('Error in BerikanVocherController::UpdateSatusPrint: ' . $e->getMessage());
                $this->sendErrorResponse('Internal server error', 500);
            } 
    }
