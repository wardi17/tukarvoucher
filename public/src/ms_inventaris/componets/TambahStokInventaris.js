...
import { goBackStok } from '../main.js';
import AttachAdd from './AttachAdd.js';
let uploadedFiles = []; 
class TambahStokInventaris {
  constructor(datas) {
    this.datas = datas;
  ...
  if (document_files.trim() !== "") {
      ...
      $.each(documentArr, function(a, b) {
        let fileURL = `${baseUrl}/uploads_attachfile/${b}`;
        const filePreview = `
...
setupDeleteEvents() {
  ...
  renderFormFields(data) {
    return `
      ${this.renderInput('InventarisID_Add', 'Inventaris ID', 'text', data.id || '', true)}
    ...
    return `
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title">Tambah Stok Inventaris</h5>
  ...
  async validateInput(event) {
    ...
    const data= {
      InventarisID: getValue('InventarisID_Add'),
      qty: getValue('qty'),
...
export default TambahStokInventaris;
